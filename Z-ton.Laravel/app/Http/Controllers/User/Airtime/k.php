...
class PurchaseAirtimeController extends Controller
{
    ...
    public function purchaseAirtime(Request $request, $userId){
        ...
        try {
                    ...
                    // $response = Http::withHeaders([ // Ensure all API keys are uncommented and loaded from .env
                    //     "api-key" => env('VTPASS_API_KEY'),
                    //     "public-key" => env('VTPASS_PUBLIC_KEY'),
                    //     "secret-key" => env('VTPASS_SECRET_KEY'),
                    // ])->post('https://sandbox.vtpass.com/api/pay', [ // Changed to POST and assumed 'api/pay' for purchase. Please verify the exact endpoint and method with VTpass API documentation.
                    //     "request_id" => $requestId, // Include required purchase parameters
                    //     "serviceID" => $serviceID,
